...
function getMaxPotentialChars(rarity) {
  ...
  Object.entries(state.inventory).forEach(([name, info]) => {
    ...
    if (data.rarity === rarity && info.potential >= 13) {
      // 检查是否在队伍中
...
function doExchange(sourceRarity, targetCharName) {
  ...
  if (sources.length < rule.cost) {
    alert(`需要${rule.cost}个${sourceRarity}星满潜干员（不在队伍中）`);
    return false;
  }
  ...
  if (targetData.rarity !== rule.target) {
    alert(`只能兑换${rule.target}星干员`);
    return false;
  }
  
  if (targetInfo.potential >= 13) {
    alert('该干员已满潜');
    return false;
  }
...
function renderExchangeUI() {
  ...
  let html = '<h3>干员兑换</h3>';
  html += '<p class="exchange-desc">消耗一定数量的满潜干员，提升高一星干员的潜能</p>';
